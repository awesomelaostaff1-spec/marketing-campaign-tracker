...
// Conditionally load puppeteer only in local environment
let puppeteer;
try {
  puppeteer = require('puppeteer');
} catch (error) {
  console.log('Puppeteer not available in serverless environment');
}
...
// Mock data for Facebook, TikTok campaigns and KOL performance
const mockData = {
  facebook: [
    {
      id: 1,
...
app.get('/', async (req, res) => {
        ...
        <div class="container">
            ...
            <div class="summary">
                ...
                <div class="export-buttons">
                    <a href="/export/pdf" class="btn btn-pdf">📄 Export PDF</a>
                    <a href="/export/pptx" class="btn btn-pptx">📊 Export PPTX</a>
                </div>
            </div>
  ...
  `);
    ...
    const totalCampaigns = mockData.facebook.length + mockData.tiktok.length + mockData.kol.length;
    const totalSpend = [...mockData.facebook, ...mockData.tiktok, ...mockData.kol].reduce((sum, item) => sum + item.cost, 0);
    const totalConversions = [...mockData.facebook, ...mockData.tiktok].reduce((sum, item) => sum + (item.conversions || 0), 0);
    ...
    res.send(`
      ...
      <head>
          ...
          <style>
              ...
              .error { background: #ff6b6b; color: white; padding: 15px; border-radius: 8px; margin-bottom: 20px; }
          </style>
      </head>
      <body>
          <div class="error">
              ⚠️ API Connection Error - Using mock data. Please configure your Facebook API credentials.
          </div>
  ...
  try {
    if (!puppeteer) {
      return res.status(503).json({
        error: 'PDF export not available in serverless environment',
        message: 'Please use the browser\'s print function (Ctrl+P) to generate PDF'
      });
    }
    ...
    await page.addStyleTag({
      content: '.export-buttons { display: none !important; }'
    });
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="marketing-performance-report.pdf"');
    res.send(pdf);
  } catch (error) {
...
// PPTX Export Route
app.get('/export/pptx', async (req, res) => {
  try {
    let pptx = new PptxGenJS();
    
    // Title Slide
    let titleSlide = pptx.addSlide();
    titleSlide.addText('🚀 Marketing Campaign Performance Report', {
      x: 0.5, y: 2, w: 9, h: 1.5,
      fontSize: 36, bold: true, color: '363636', align: 'center'
    });
    titleSlide.addText('Facebook, TikTok & KOL Analytics for Laos Market', {
    ...
    summarySlide.addText(`Total Campaigns: ${totalCampaigns}`, {
      x: 1, y: 2, w: 8, h: 0.6,
      fontSize: 18, color: '363636'
    });
    summarySlide.addText(`Total Spend: $${totalSpend.toFixed(2)}`, {
      x: 1, y: 2.8, w: 8, h: 0.6,
      fontSize: 18, color: '363636'
    });
    summarySlide.addText(`Total Conversions: ${totalConversions}`, {
      x: 1, y: 3.6, w: 8, h: 0.6,
      fontSize: 18, color: '363636'
    });

    // Facebook Campaigns Slide
    let facebookSlide = pptx.addSlide();
    facebookSlide.addText('📘 Facebook Campaigns', {
      x: 0.5, y: 0.5, w: 9, h: 0.8,
      fontSize: 28, bold: true, color: '1877f2'
    });
    
    let fbTableData = [
      ['Campaign', 'Impressions', 'Clicks', 'Cost', 'Conversions', 'CTR']
    ];
    mockData.facebook.forEach(campaign => {
      fbTableData.push([
        campaign.name,
    ...
    facebookSlide.addTable(fbTableData, {
      x: 0.5, y: 1.5, w: 9, h: 3,
      fontSize: 12, border: {pt: 1, color: 'CFCFCF'}
    });

    // TikTok Campaigns Slide
    let tiktokSlide = pptx.addSlide();
    tiktokSlide.addText('🎵 TikTok Campaigns', {
      x: 0.5, y: 0.5, w: 9, h: 0.8,
      fontSize: 28, bold: true, color: 'ff0050'
    });
    
    let ttTableData = [
      ['Campaign', 'Impressions', 'Clicks', 'Cost', 'Likes', 'Shares']
    ];
    mockData.tiktok.forEach(campaign => {
      ttTableData.push([
        campaign.name,
        campaign.impressions.toLocaleString(),
        campaign.clicks.toLocaleString(),
        ...
        campaign.shares.toLocaleString()
      ]);
    });
    
    tiktokSlide.addTable(ttTableData, {
      x: 0.5, y: 1.5, w: 9, h: 3,
      fontSize: 12, border: {pt: 1, color: 'CFCFCF'}
    });

    // KOL Performance Slide
    let kolSlide = pptx.addSlide();
    kolSlide.addText('🌟 KOL Performance (TikTok)', {
      x: 0.5, y: 0.5, w: 9, h: 0.8,
      fontSize: 28, bold: true, color: '25d366'
    });
    
    let kolTableData = [
      ['KOL Name', 'Followers', 'Engagement%', 'Posts', 'Total Likes', 'Cost']
    ];
    mockData.kol.forEach(kol => {
      kolTableData.push([
        kol.name,
    ...
    kolSlide.addTable(kolTableData, {
      x: 0.5, y: 1.5, w: 9, h: 3,
      fontSize: 12, border: {pt: 1, color: 'CFCFCF'}
    });

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
    res.setHeader('Content-Disposition', 'attachment; filename="marketing-performance-report.pptx"');
    ...
    res.status(500).send('Error generating PPTX report');
  }
});
...
app.get('/api/summary', async (req, res) => {
  try {
    const adAccountId = req.query.account_id;
    const facebookData = await facebookAPI.getCampaignInsights(adAccountId);
    const tiktokData = mockData.tiktok;
    const kolData = mockData.kol;
    ...
    res.json({
      ...
      totalConversions,
      platforms: {
        facebook: mockData.facebook.length,
        tiktok: mockData.tiktok.length,
        kol: mockData.kol.length
      }
    });
  }
});
...
app.listen(port, () => {
  console.log(`🎉 Marketing Campaign Tracker running at http://localhost:${port}`);
  console.log(`🔗 Click the "Open in Browser" button that appears!`);
});
